#!/usr/bin/env python3
"""Upload resume-tailoring suggestions to Google Sheets."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Upload resume tailoring JSON to a Google Sheet tab."
    )
    parser.add_argument("--json", required=True, help="Path to resume_tailoring JSON file")
    parser.add_argument("--sheet", required=True, help="Google Sheet URL or spreadsheet ID")
    parser.add_argument("--tab", default="resume_tailoring", help="Target sheet tab name")
    parser.add_argument(
        "--mode",
        default="replace",
...
def build_records(payload: Dict[str, Any], source_file: str) -> List[Dict[str, str]]:
    rows_in = payload.get("tailoring", [])
    if not isinstance(rows_in, list):
        raise ValueError("Expected 'tailoring' to be a list")
...
def main() -> int:
    ...
    if not records:
        print("No resume tailoring rows in JSON. Nothing to upload.")
        return 0
    ...
    print(f"Uploaded {uploaded} resume-tailoring rows to '{args.tab}' in spreadsheet {sheet_id}.")
    return 0
