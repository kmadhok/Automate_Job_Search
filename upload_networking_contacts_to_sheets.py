#!/usr/bin/env python3
"""Upload networking contact candidates to Google Sheets."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Upload networking contact JSON to a Google Sheet tab."
    )
    ...
    parser.add_argument("--tab", default="networking_contacts", help="Target sheet tab name")
    parser.add_argument(
        "--mode",
        default="replace",
...
def build_records(payload: Dict[str, Any], source_file: str) -> List[Dict[str, str]]:
    ...
    if not isinstance(groups, list):
        raise ValueError("Expected 'search_groups' to be a list")
    ...
    for group in groups:
        ...
        for idx, person in enumerate(candidates, start=1):
            ...
            row = {
                ...
                "matched_title_terms": person.get("matched_title_terms"),
                "matched_keywords": person.get("matched_keywords"),
                "matched_focus_terms": person.get("matched_focus_terms"),
            }
...
def main() -> int:
    ...
    if not records:
        print("No networking contacts in JSON. Nothing to upload.")
        return 0
    ...
    print(f"Uploaded {uploaded} networking contact rows to '{args.tab}' in spreadsheet {sheet_id}.")
    return 0
